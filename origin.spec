...
Obsoletes: openshift >= 1.0
Obsoletes: openshift <= 3.1
...
if [ ! -e %{_sysconfdir}/origin/master/master-config.yaml ]; then
  # Let the ansible installer know these files were RPM generated
%if "%{dist}" == ".el7aos"
...
%endif
  ...
  echo "Configuration Generated by RPM at `date -u`" > %{_sysconfdir}/origin/.config_managed
fi
