...
for cmd in oc oadm kubectl atomic-enterprise; do
    ln -s %{_bindir}/openshift %{buildroot}%{_bindir}/$cmd
done

install -d -m 0755 %{buildroot}%{_sysconfdir}/origin/{master,node}

install -m 0644 rel-eng/origin-master.service %{buildroot}%{_unitdir}/%{name}-master.service
install -m 0644 rel-eng/origin-node.service %{buildroot}%{_unitdir}/%{name}-node.service
